...
| [Plugin marketplace control](plugin-marketplaces.md#managed-marketplace-restrictions) | Restrict which marketplace sources users can add and install from, reject the CLI flags that sideload plugins, agents, and MCP servers for a single run, and allowlist which marketplaces' plugins can be suggested                                                          | `strictKnownMarketplaces`, `blockedMarketplaces`, `disableSideloadFlags`, `pluginSuggestionMarketplaces`     |
...
| [Login enforcement](settings.md#available-settings)                                   | Restrict interactive login to a specific method or Anthropic organization. When set, sessions authenticated by `ANTHROPIC_API_KEY`, `ANTHROPIC_AUTH_TOKEN`, or `apiKeyHelper` are blocked at startup; cloud provider sessions aren't affected                                | `forceLoginMethod`, `forceLoginOrgUUID`                                                                      |
...
| [Configure the corporate launcher](corporate-launcher.md)                             | Prefix the [background-agent supervisor](agent-view.md#how-background-sessions-are-hosted), its workers, and the [other covered background processes](corporate-launcher.md#what-the-launcher-covers) with a required corporate launcher instead of turning agent view off | `processWrapper`                                                                                             |
...
[Claude Code on the web](claude-code-on-the-web.md) has its own admin surface: on the Cloud environments page in admin settings, owners and admins create [organization-shared environments](claude-code-on-the-web.md#organization-shared-environments) that set the [network access level](claude-code-on-the-web.md#network-access), environment variables, and setup script for members' cloud sessions, and choose the organization's default environment.
