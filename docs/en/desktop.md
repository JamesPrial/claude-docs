...
* **Model**: pick a [model](overview.md#models) from the dropdown next to the send button. The model is locked once the session starts.
...
The `delegate` and `dontAsk` permission modes are available only in the [CLI](permissions.md#permission-modes).

<Tip title="Best practice">
  Start complex tasks in Plan mode so Claude maps out an approach before making changes. Once you approve the plan, switch to Code or Ask mode to execute it. See [explore first, then plan, then code](best-practices.md#explore-first-then-plan-then-code) for more on this workflow.
</Tip>
...
[Extended thinking](common-workflows.md#use-extended-thinking-thinking-mode) is enabled by default, which improves performance on complex reasoning tasks but uses additional tokens. To disable it or adjust the budget, set `MAX_THINKING_TOKENS` in your shell profile. Use `0` to disable.
...
| Key                                        | Description                                                                                                      |
| ------------------------------------------ | ---------------------------------------------------------------------------------------------------------------- |
| `permissions.disableBypassPermissionsMode` | set to `"disable"` to prevent users from enabling Act mode. See [permissions](permissions.md#managed-settings). |
