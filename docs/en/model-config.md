...
* Sessions on [third-party providers](server-managed-settings.md#platform-availability) such as Bedrock, Vertex AI, Foundry, and [Claude Platform on AWS](claude-platform-on-aws.md) do not receive server-managed settings, so deliver the allowlist through MDM or managed settings files there.
