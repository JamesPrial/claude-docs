...
| **`sonnet[1m]`** | Uses Sonnet with a [1 million token context window](https://platform.claude.com/docs/en/build-with-claude/context-windows#1m-token-context-window) for long sessions. No effect when `sonnet` already resolves to Sonnet 5 with its native 1M window; behind an [LLM gateway](llm-gateway.md), selects the 1M window for Sonnet 5 |
...
| `CLAUDE_CODE_SUBAGENT_MODEL`     | The model to use for all [subagents](sub-agents.md#choose-a-model) and [agent teams](agent-teams.md). Overrides the per-invocation `model` parameter and the subagent definition's `model` frontmatter. Set to `inherit` to use normal model resolution instead |
