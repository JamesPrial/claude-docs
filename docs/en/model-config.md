...
| `CLAUDE_CODE_SUBAGENT_MODEL`     | The model to use for all [subagents](sub-agents.md#choose-a-model). Overrides both the per-invocation `model` parameter and the subagent definition's `model` frontmatter |
