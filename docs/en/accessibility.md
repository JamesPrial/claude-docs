...
<Note>
  The methods are listed in precedence order: the [`--ax-screen-reader`](cli-reference.md#cli-flags) flag overrides the [`CLAUDE_AX_SCREEN_READER`](env-vars.md) environment variable, which overrides the [`axScreenReader`](settings.md#available-settings) setting.
</Note>
...
Screen reader mode renders as plain scrolling text, even if you've turned on [fullscreen rendering](fullscreen.md) with the [`tui` setting](settings.md#available-settings); the setting has no effect while the mode is active. Attached background sessions still render fullscreen; see [Known limitations](#known-limitations).
...
{/* min-version: 2.1.210 */}Cycling [permission modes](permission-modes.md) with `Shift+Tab` announces the mode you land on, such as `[plan mode on]` or `[accept edits on]`. Claude Code prints the announcement once and doesn't repeat it on later redraws. Requires Claude Code v2.1.210 or later.
...
The bell is your terminal's standard alert. To silence it, change the bell setting in your terminal application. The bell doesn't require screen reader mode: outside the mode, set [`preferredNotifChannel`](settings.md#available-settings) to `"terminal_bell"` for similar alerts when Claude is waiting on you. See [Get a terminal bell or notification](terminal-config.md#get-a-terminal-bell-or-notification).
...
* The `prefersReducedMotion` [setting](settings.md#available-settings) reduces or disables spinners, shimmer, and other animations without changing the rest of the interface.
* The `theme` [setting](settings.md#available-settings) selects the interface colors, including the colorblind-friendly `dark-daltonized` and `light-daltonized` themes.
...
* [Settings](settings.md#available-settings): the `axScreenReader`, `prefersReducedMotion`, `theme`, and `preferredNotifChannel` entries
* [Environment variables](env-vars.md): the `CLAUDE_AX_SCREEN_READER` and `CLAUDE_CODE_ACCESSIBILITY` entries
* [CLI reference](cli-reference.md#cli-flags): the `--ax-screen-reader` flag
