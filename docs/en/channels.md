...
* Claude Code [installed and authenticated](quickstart.md#step-1-install-claude-code) with a claude.ai account
...
If Claude hits a permission prompt while you're away from the terminal, the session pauses until you approve locally. For unattended use, [`--dangerously-skip-permissions`](permissions.md#permission-modes) bypasses prompts, but only use it in environments you trust.
