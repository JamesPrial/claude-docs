...
Because the hook replaces the default behavior entirely, [`.worktreeinclude`](common-workflows.md#copy-gitignored-files-to-worktrees) is not processed. If you need to copy local configuration files like `.env` into the new worktree, do it inside your hook script.
