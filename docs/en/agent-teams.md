...
From there, Claude populates a [shared task list](interactive-mode.md#task-list), spawns teammates for each perspective, has them explore the problem, and synthesizes findings when finished.
...
To override the default, set [`teammateMode`](settings.md#available-settings) in `~/.claude/settings.json`:
...
{/* min-version: 2.1.186 */}Teammates inherit the lead's [effort level](model-config.md#adjust-effort-level). In split-pane mode this applies from v2.1.186; earlier versions did not pass the lead's session effort to split-pane teammates.
...
Claude Code generates both of these automatically at session startup and updates them as teammates join, go idle, or leave. The team config directory is removed when the session ends. The task list directory persists locally and is never uploaded, so resumed sessions keep their tasks. Retention is governed by the same [`cleanupPeriodDays`](settings.md#available-settings) you already control for session transcripts.
...
Having 5-6 [tasks](agent-teams.md#architecture) per teammate keeps everyone productive without excessive context switching. If you have 15 independent tasks, 3 teammates is a good starting point.
...
* **Compare approaches**: see the [subagent vs agent team](features-overview.md#compare-similar-features) comparison for a side-by-side breakdown
