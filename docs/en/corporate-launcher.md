...
<Note>
  `CLAUDE_CODE_PROCESS_WRAPPER` requires Claude Code v2.1.208 or later. Earlier versions ignore the variable and start every process unwrapped. {/* min-version: 2.1.210 */}The equivalent [`processWrapper` setting](settings.md#available-settings) requires v2.1.210 or later. Earlier versions ignore it as an unknown key, apply no launcher, and report no error.
...
* {/* min-version: 2.1.210 */}The [Remote Control](remote-control.md) worker processes the background service manages. Requires Claude Code v2.1.210 or later.
* {/* min-version: 2.1.210 */}The split-pane teammate sessions that [agent teams](agent-teams.md) start in tmux or iTerm2. Teammate panes are interactive rather than background processes, but Claude Code starts them from its own binary, so the launcher covers them. Requires Claude Code v2.1.210 or later.
...
<Steps>
  ...
  <Step title="Set CLAUDE_CODE_PROCESS_WRAPPER in settings">
    ...
    For one machine, add it to `~/.claude/settings.json`. To deploy it to every machine in your organization, put the same block in [managed settings](permissions.md#managed-settings):
    ...
    Because `processWrapper` is a named setting, an organization that delivers it through [remote managed settings](settings.md#settings-files) sees it listed on the [security approval dialog](server-managed-settings.md#security-approval-dialogs) alongside the other settings that run administrator-supplied executables.
