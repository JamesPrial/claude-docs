...
* `safe_mode`: `"true"` when the session was started with [`--safe-mode`](cli-reference.md), `"false"` otherwise. In safe mode this event reports configured inventory only; the plugin's commands, skills, hooks, and MCP servers don't load. {/* min-version: 2.1.169 */}Requires Claude Code v2.1.169 or later
