...
* **User confirmation**: New domain requests trigger permission prompts (unless [`allowManagedDomainsOnly`](settings.md#sandbox-settings) is enabled, which blocks non-allowed domains automatically)
...
* **Computer use**: when Claude opens apps and controls your screen on macOS, it runs on your actual desktop rather than in an isolated environment. Per-app permission prompts gate each application. See [computer use in the CLI](computer-use.md) or [computer use in Desktop](desktop.md#let-claude-use-your-computer).
