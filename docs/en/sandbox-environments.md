...
* **Built-in Bash sandbox**: the only approach Claude Code enforces itself. Deliver the `sandbox` settings keys through [managed settings](settings.md#settings-files), either as a file managed by your MDM or through [server-managed settings](server-managed-settings.md) on Claude.ai. See [Enforce sandboxing with managed settings](sandboxing.md#enforce-sandboxing-with-managed-settings) for the keys to deploy and how to keep developers from widening the policy.
...
* [Settings](settings.md#sandbox-settings): all sandbox configuration keys, including managed settings delivery
