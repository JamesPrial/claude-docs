...
The container exposes an HTTP or WebSocket endpoint and maps each active session to a long-lived query and the subprocess behind it. In TypeScript, use [`streamInput()`](agent-sdk/typescript.md#query-object) to add turns to an active session and [`startup()`](agent-sdk/typescript.md#startup) to pre-warm subprocesses ahead of incoming traffic. In Python, use [`ClaudeSDKClient`](agent-sdk/python.md#claudesdkclient) to hold a session open across turns. Size the container so it can hold the maximum number of concurrent sessions in memory.
...
For self-hosted options such as Docker, gVisor, and Firecracker, and detailed isolation configuration, see [Isolation Technologies](agent-sdk/secure-deployment.md#isolation-technologies).
...
Default local disk is lost on restart, scale-down, or a move to a different node. For any session a user expects to resume, mirror the transcript to durable storage with a [`SessionStore` adapter](agent-sdk/session-storage.md). See [Reference implementations](agent-sdk/session-storage.md#reference-implementations) for S3, Redis, and Postgres adapters and a conformance suite for your own.
...
Prompt text and tool inputs are not included in exports by default. See [Control sensitive data in exports](agent-sdk/observability.md#control-sensitive-data-in-exports) for the opt-in flags, and [Observability](agent-sdk/observability.md) for the full signal catalog.

### Auth and secrets

Three auth concerns matter at hosting time:

* **Anthropic API**: the subprocess reads `ANTHROPIC_API_KEY` from its environment. Supply it from your secret manager, or set `ANTHROPIC_BASE_URL` to route model calls through a proxy that injects the key outside the container. See [Credential management](agent-sdk/secure-deployment.md#credential-management) for the proxy pattern and the [SDK overview](agent-sdk/overview.md#get-started) for supported authentication methods.
