...
| `session_store`               | [`SessionStore`](agent-sdk/session-storage.md#the-session-store-interface) ` \| None` | `None`                             | Mirror session transcripts to an external backend so any host can resume them. See [Persist sessions to external storage](agent-sdk/session-storage.md)                                                                                                                  |
...
| `exclude_dynamic_sections` | No       | Move per-session context such as working directory, git status, and memory paths from the system prompt into the first user message. Improves prompt-cache reuse across users and machines. See [Modify system prompts](agent-sdk/modifying-system-prompts.md#improve-prompt-caching-across-users-and-machines) |
...
The `model_usage` dict maps model names to per-model usage. The inner dict keys use camelCase because the value is passed through unmodified from the underlying CLI process, matching the TypeScript [`ModelUsage`](agent-sdk/typescript.md#model-usage) type:
...
<Note>
  **Filesystem and network access restrictions** are NOT configured via sandbox settings. Instead, they are derived from [permission rules](settings.md#permission-settings):
