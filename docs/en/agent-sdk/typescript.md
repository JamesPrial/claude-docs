...
<Note>
  `TodoWrite` is deprecated and will be removed in a future release. Use `TaskCreate`, `TaskGet`, `TaskUpdate`, and `TaskList` instead. Set `CLAUDE_CODE_ENABLE_TASKS=1` to opt in. See [Migrate to Task tools](agent-sdk/todo-tracking.md#migrate-to-task-tools) for how monitoring code changes.
</Note>
