...
Custom tools default to sequential execution. To enable parallel execution for a custom tool, mark it as read-only in its annotations: `readOnly` in [TypeScript](agent-sdk/typescript.md#tool) or `readOnlyHint` in [Python](agent-sdk/python.md#tool).

## Control how the loop runs

You can limit how many turns the loop takes, how much it costs, how deeply Claude reasons, and whether tools require approval before running. All of these are fields on [`ClaudeAgentOptions`](agent-sdk/python.md#claude-agent-options) (Python) / [`Options`](agent-sdk/typescript.md#options) (TypeScript).
...
When either limit is hit, the SDK returns a `ResultMessage` with a corresponding error subtype (`error_max_turns` or `error_max_budget_usd`). See [Handle the result](#handle-the-result) for how to check these subtypes and [`ClaudeAgentOptions`](agent-sdk/python.md#claude-agent-options) / [`Options`](agent-sdk/typescript.md#options) for syntax.
...
* **Be selective with tools.** Every tool definition takes context space. Use the `tools` field on [`AgentDefinition`](agent-sdk/subagents.md#agent-definition-configuration) to scope subagents to the minimum set they need, and use [MCP tool search](agent-sdk/mcp.md#mcp-tool-search) to load tools on demand instead of preloading all of them.
...
<Note>
  In Python, `ClaudeSDKClient` handles session IDs automatically across multiple calls. See the [Python SDK reference](agent-sdk/python.md#choosing-between-query-and-claude-sdk-client) for details.
</Note>
