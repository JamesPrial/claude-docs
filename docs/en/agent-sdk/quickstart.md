...
| `dontAsk`           | Denies anything not in `allowedTools`; connector tools [your organization set to `ask`](mcp.md#organization-controls-on-connector-tools) and tools that require user interaction are denied even if you've listed them                                                                                                                                                              | Locked-down headless agents               |
| `auto`              | A model classifier approves or denies each tool call                                                                                                                                                                                                                                                                                                                                 | Autonomous agents with safety guardrails  |
| `bypassPermissions` | Runs every tool without prompting, except tools matched by an explicit [`ask` rule](agent-sdk/permissions.md#how-permissions-are-evaluated), connector tools [your organization set to `ask`](mcp.md#organization-controls-on-connector-tools), and tools that require user interaction. In the TypeScript SDK, also requires `allowDangerouslySkipPermissions: true` in `options` | Sandboxed CI, fully trusted environments  |
