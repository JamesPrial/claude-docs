...
| `bypassPermissions` | Runs every tool without prompting, except tools matched by an explicit [`ask` rule](agent-sdk/permissions.md#how-permissions-are-evaluated), connector tools [your organization set to `ask`](mcp.md#organization-controls-on-connector-tools), and tools that require user interaction | Sandboxed CI, fully trusted environments  |
