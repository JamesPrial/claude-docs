...
* **Tools loaded into the prefix**: any change to them invalidates the cache. This happens when [tool search is unavailable or disabled](mcp.md#configure-tool-search), such as on Haiku models, on Google Cloud's Agent Platform, or with a custom `ANTHROPIC_BASE_URL` gateway. It also happens for a server or tool marked [`alwaysLoad`](mcp.md#exempt-a-server-from-deferral), and for definitions kept upfront by [threshold-based loading](mcp.md#configure-tool-search).
