...
* **Effort level**: not part of the cache key or the prompt, so [changing it](model-config.md#adjust-effort-level) mid-session has no effect on the cache.
...
[MCP tool search](mcp.md#scale-with-mcp-tool-search) reduces how much each tool contributes to the prefix by deferring full tool definitions, but the set of tool names still has to stay stable for the cache to remain valid.
