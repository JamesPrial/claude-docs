...
| :------------------------------------------- | :------ | :------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`CLAUDE_CODE_MAX_RETRIES`](env-vars.md)    | 10      | Number of retry attempts. {/* min-version: 2.1.186 */}Capped at 15 as of v2.1.186. Lower it to surface failures faster in scripts.                                   |
...
The model that [auto mode](permission-modes.md#eliminate-prompts-with-auto-mode) uses to classify actions could not produce a decision, so auto mode did not approve the action automatically. The message you see depends on why the classifier failed.
...
* This is transient and unrelated to [auto mode eligibility](permission-modes.md#eliminate-prompts-with-auto-mode); you do not need to change settings
...
* See [Authentication precedence](authentication.md#authentication-precedence) to understand which credential wins when several are present
...
Environment variables and `apiKeyHelper` take precedence over `/login`, so running `/login` alone does not help while either is still supplying a key. See [Authentication precedence](authentication.md#authentication-precedence).
