...
If you are prompted to log in repeatedly, see [Not logged in or token expired](troubleshooting.md#not-logged-in-or-token-expired) for system clock and macOS Keychain fixes.
...
* For repeated prompts to log in across launches, see the system clock and macOS Keychain checks in [Troubleshooting](troubleshooting.md#not-logged-in-or-token-expired)
* For other failures including `403 Forbidden` and OAuth browser issues, see [Permissions and authentication](troubleshooting.md#permissions-and-authentication)
...
* See [Model not found](troubleshooting.md#model-not-found-or-not-accessible) to locate where a stale ID is set across `--model`, `ANTHROPIC_MODEL`, and settings files
...
* See [Extended thinking](common-workflows.md#use-extended-thinking-thinking-mode) for how the budget interacts with output length
...
* **Effort level**: run `/effort` to check the current reasoning level and raise it for hard debugging or design work. Defaults vary by model and plan, so check before assuming you are below the maximum. See [Adjust effort level](model-config.md#adjust-effort-level) for per-model defaults and the `ultrathink` shortcut.
