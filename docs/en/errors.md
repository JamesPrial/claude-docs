...
When a rate limit, overload, or server error interrupts a foreground subagent that already produced output, Claude receives that partial output marked as incomplete instead of this error. See [API errors in subagents](sub-agents.md#api-errors-in-subagents).
...
This appears on Amazon Bedrock, Google Vertex AI, and Microsoft Foundry. {/* min-version: 2.1.196 */}As of v2.1.196 it also appears when [`ANTHROPIC_BASE_URL`](env-vars.md) points at a host other than `api.anthropic.com`, such as an [LLM gateway](llm-gateway.md) or proxy, even when you sign in with claude.ai.
...
* For Vertex AI deployments, see [Vertex AI troubleshooting](google-vertex-ai.md#troubleshooting).
