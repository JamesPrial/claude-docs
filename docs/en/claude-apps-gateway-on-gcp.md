...
<Steps>
  ...
  <Step title="Write gateway.yaml">
    The `upstreams` block points at Agent Platform with `auth: {}`, so the gateway authenticates via Application Default Credentials from the runtime service account. See the [configuration reference](claude-apps-gateway-config.md) for every field.
