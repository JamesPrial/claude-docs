...
Letting Claude jump straight to coding can produce code that solves the wrong problem. Use [Plan Mode](common-workflows.md#use-plan-mode-for-safe-code-analysis) to separate exploration from execution.
...
<Tip>
  Use [auto mode](permission-modes.md#eliminate-prompts-with-auto-mode) to let a classifier handle approvals, `/permissions` to allowlist specific commands, or `/sandbox` for OS-level isolation. Each reduces interruptions while keeping you in control.
</Tip>
...
* For quick questions that don't need to stay in context, use [`/btw`](interactive-mode.md#side-questions-with-btw). The answer appears in a dismissible overlay and never enters conversation history, so you can check a detail without growing context.
...
* [Claude Code desktop app](desktop.md#work-in-parallel-with-sessions): Manage multiple local sessions visually. Each session gets its own isolated worktree.
...
For non-interactive runs with the `-p` flag, auto mode aborts if the classifier repeatedly blocks actions, since there is no user to fall back to. See [when auto mode falls back](permission-modes.md#when-auto-mode-falls-back) for thresholds.
