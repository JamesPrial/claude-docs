...
* **Managed settings**: For organizations that need centralized control, Claude Code supports `managed-settings.json` and `managed-mcp.json` files that can be deployed to system directories:
  ...
  * Windows: `C:\Program Files\ClaudeCode\`
  ...
  See [Managed settings](permissions.md#managed-settings) and [Managed MCP configuration](mcp.md#managed-mcp-configuration) for details. For organizations without device management infrastructure, see [server-managed settings](server-managed-settings.md).
...
| `disableBypassPermissionsMode` | Set to `"disable"` to prevent `bypassPermissions` mode from being activated. This disables the `--dangerously-skip-permissions` command-line flag. See [managed settings](permissions.md#managed-settings)                                      | `"disable"`                                                            |
...
1. **Managed settings** ([`managed-settings.json`](permissions.md#managed-settings) or [server-managed settings](server-managed-settings.md))
...
**Managed settings only**: Controls which plugin marketplaces users are allowed to add. This setting can only be configured in [`managed-settings.json`](permissions.md#managed-settings) and provides administrators with strict control over marketplace sources.
