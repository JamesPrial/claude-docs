...
In print mode they are off by default. Pass [`--prompt-suggestions`](cli-reference.md#cli-flags) with `--output-format stream-json --verbose` to emit a `prompt_suggestion` message after each turn instead.
