...
Either method works. [`/schedule`](web-scheduled-tasks.md) checks for either form of access and prompts you to run `/web-setup` if neither is configured. See [Connect from your terminal](web-quickstart.md#connect-from-your-terminal) for the `/web-setup` walkthrough.
...
* **From your terminal**: run [`/autofix-pr`](commands.md#built-in-commands) while on the PR's branch. Claude Code detects the open PR with `gh`, spawns a web session, and turns on auto-fix in one step
