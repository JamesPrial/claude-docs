...
Claude Code supports a subset of Vim keybindings that can be enabled via `/config` → Editor mode. To set the mode directly in your config file, set the [`editorMode`](settings.md#global-config-settings) global config key to `"vim"` in `~/.claude.json`.
