...
When Claude finishes working and is waiting for your input, it fires a notification event. You can surface this event as a desktop notification through your terminal or run custom logic with [notification hooks](hooks.md#notification).
...
When running Claude Code inside tmux, notifications and the [terminal progress bar](settings.md#global-config-settings) only reach the outer terminal, such as iTerm2, Kitty, or Ghostty, if you enable passthrough in your tmux configuration:
...
Other terminals, including the default macOS Terminal, do not support native notifications. Use [notification hooks](hooks.md#notification) instead.

#### Notification hooks

To add custom behavior when notifications fire, such as playing a sound or sending a message, configure a [notification hook](hooks.md#notification). Hooks run alongside terminal notifications, not as a replacement.
...
Claude Code supports a subset of Vim keybindings that can be enabled with `/vim` or configured via `/config`. To set the mode directly in your config file, set the [`editorMode`](settings.md#global-config-settings) global config key to `"vim"` in `~/.claude.json`.
